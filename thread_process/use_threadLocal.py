...
每个函数一层一层调用都这么传参数那还得了？用全局变量也不行，因为每个线程处理不同的Student对象，不能共享。
...
有没有更简单的方式？ ThreadLocal 应运而生，不用查找dict，ThreadLocal帮你自动做这件事：
'''
import threading

local_school = threading.local()    # 全局变量ThreadingLocal对象
print("-------->", local_school)
def process_student():
