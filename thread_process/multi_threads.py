...
def run_thread(n):
    ...
    for i in range(1000000):
        change_it(n)
    print("thread %s used %0.2f."% (threading.current_thread().name, (time.time() - start)))
...
t1.join()
t2.join()
print("\t两个线程交替执行，次数足够多的时候，值会被影响：", balance)
'''
...
当某个线程开始执行change_it()时，该线程被锁定，因此其他线程不能同时执行change_it()，只能等待，直到锁被释放后，获得该锁以后才能改。
...
def run_thread2(n):
    start = time.time()
    for i in range(1000000):
        lock.acquire()      # 获取锁，只有一个线程可以成功获取
        try:
            change_it(n)
        finally:
            lock.release()  # 注意，一定要释放。否则其他线程只有等着
    print("thread %s used %0.2f."% (threading.current_thread().name, (time.time() - start)))

tt1 = threading.Thread(target=run_thread2, name="tt1", args=(5,))
tt2 = threading.Thread(target=run_thread2, name="tt2", args=(8,))
tt1.start()
tt2.start()
tt1.join()
tt2.join()
print("\t给线程加上锁后，保证一个线程可以从头到尾完整执行后，其他线程才执行：", balance)
'''
