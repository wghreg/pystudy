...
print(list(filter(isblank, ["A", "", "B", None, "C", " "])))
...
取新序列的第一个数5，然后用5把序列的5的倍数筛掉：7, 11, 13, 17, 19, ...
...
print("\n0-1000中的回数为：", list(output))

def is_palindrome2(n):
    s = str(n)
    for i in range(len(s)):
       return s[i] == s[i-1]
print(list(filter(is_palindrome2, range(1,1000))))

def is_palindrome3(n):
    return n - int( str(n)[::-1] ) == 0
print(list(filter(is_palindrome3, range(1,1000))))
